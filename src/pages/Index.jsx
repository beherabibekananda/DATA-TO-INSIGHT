    ...
    <div className="min-h-screen bg-[#02040a] relative overflow-hidden font-sans selection:bg-primary/30 selection:text-white">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full opacity-20"
          style={{ background: 'radial-gradient(circle at 50% 50%, rgba(139, 92, 246, 0.05) 0%, transparent 70%)' }}></div>
      </div>
      ...
      `}>
        {/* Intelligence Header */}
        <header className="sticky top-0 z-40 bg-card/10 backdrop-blur-2xl border-b border-white/5 px-10 py-6">
          <div className="flex items-center justify-between mx-auto max-w-7xl">
            <div className="flex items-center gap-8">
              <div className="hidden lg:flex items-center gap-4 bg-white/[0.02] px-4 py-2 rounded-2xl border border-white/5 shadow-xl">
                <div className="flex items-center gap-2">
                  <Cpu className="w-3.5 h-3.5 text-primary animate-spin-slow" />
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Core Engine: </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-success rounded-full shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div>
                  <span className="text-success text-[10px] font-black uppercase tracking-widest">Optimized</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 text-white/30">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">
                  {currentTime.toLocaleDateString('en-US', { weekday: 'short' })} • {currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              {user && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === 'requests' ? 'bg-primary/10 text-primary border border-primary/20' : 'text-white/40 hover:text-white hover:bg-white/[0.03]'}`}
                  >
                    Directives
                  </button>
                  <button
                    onClick={() => setActiveSection('user-dashboard')}
                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === 'user-dashboard' ? 'bg-primary/10 text-primary border border-primary/20' : 'text-white/40 hover:text-white hover:bg-white/[0.03]'}`}
                  >
                    Neural Profile
                  </button>
                </div>
              ...
              <div className="flex items-center gap-4">
                <button className="relative w-11 h-11 flex items-center justify-center bg-white/[0.02] rounded-2xl border border-white/5 hover:bg-white/[0.08] transition-all group">
      ...
      <div className="fixed inset-0 bg-[#02040a] z-[100] flex items-center justify-center transition-all duration-1000 pointer-events-none opacity-0 animate-init-overlay">
        <div className="text-center relative">
          <div className="absolute inset-x-0 -top-20 flex justify-center">
            <div className="w-32 h-32 bg-primary/20 blur-[60px] animate-pulse rounded-full"></div>
          </div>
          <div className="relative z-10 group">
            <div className="w-24 h-24 border-2 border-primary/10 border-t-primary rounded-full animate-spin mb-8 mx-auto p-4 transition-all group-hover:scale-110">
            ...
            <div className="h-1 w-48 bg-white/5 mx-auto rounded-full overflow-hidden mb-4 pr-1">
              <div className="h-full bg-primary w-0 animate-progress"></div>
            </div>
            <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.3em] animate-pulse">Initializing Neural Nodes</p>
          </div>
        </div>
      </div>
